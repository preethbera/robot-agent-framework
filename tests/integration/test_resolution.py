...
def test_resolution_and_deterministic_artifacts(tmp_path: Path) -> None:
    ...
    result = next(
        channel for channel in first.agent.channels if channel.id == "command.result"
    )
    ...
    assert first.definition.ros2_overrides == {
        "temperature": {"name": "/native/temperature"}
    }
...
def test_same_binding_can_be_exposed_twice() -> None:
    ...
    assert {"temperature.value", "second.value"} <= {
        c.id for c in resolution.agent.channels
    }
...
def test_mandatory_constraint_survives_omission() -> None:
    ...
    mandatory = next(
        item
        for item in resolution.agent.constraints
        if item.id == "command.request_range"
    )
...
def test_configurable_defaults_are_not_automatically_mandatory() -> None:
    ...
    assert (
        next(c for c in result.agent.channels if c.id == "lidar.output").timing
        is not None
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        SOURCE.replace(
            "parameters: {max: 80}", "parameters: {max: 80, field: missing}"
        ),
...
def test_artifact_path_traversal_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ResolutionError, match="unsafe"):
        write_artifacts(
            replace(result, agent=replace(result.agent, id="../escape")), tmp_path
        )
...
def test_required_allowed_values_cannot_be_widened() -> None:
    ...
    required = next(
        iter(resolve_agent(parse_agent_definition(SOURCE)).agent.constraints)
    )
    required = replace(
        required, kind=ConstraintKind.ALLOWED_VALUES, parameters={"values": [1, 2]}
    )
